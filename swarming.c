//This code simulates the swarming of a school of fish in a light field.
//Reinforcement Learning is used to train the school of fish to optimize the swarming behavior.
// The aim is to see if several different fractions of the population having different interaction leads to any interesting segregation phenomena
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include "ran2.c"
...
// Generating the field parameters
void get_field_amps(int time)
{
    int i;
    FILE *fpw;
    //fpw=fopen("field_amp01.dat","a");
    double Bmean,Bsd;
    Bmean=0.4; Bsd=0.1;
    for(i=0;i<Knum;i++)
    {
...
void update_posvel(int time,int step, double ro, double ra,int ind)
{
    int i,k;
...
double train(int step, double ro, double ra, int ind)
{
    FILE *fpw;
    //fpw=fopen("particle_pos.dat","w");
    int i,k;
    double tot_fl0,tot_fl1,Q;
    tot_fl0=0;
    for(i=0;i<N;i++)
        tot_fl0=tot_fl0+evaluate_field(i);
    tot_fl0=tot_fl0/N;
 
    for(i=0;i<tstar;i++)
    {
        update_posvel(i,step,ro, ra, ind);
        //fprintf(fpw,"%d %lf %lf\n",i,r[5][0],r[5][1]);
    }
      tot_fl1=0;
    
    for(i=0;i<N;i++)
        tot_fl1=tot_fl1+evaluate_field(i);
...
int main()
{
    ...
    for(i=1;i<=Ntrain;i++)
    {
        ...
        delta=1.0/pow((i+1.0),0.25);
        gamma=1.0/pow((i+1.0),0.75);

        //restore_initialize();
        Q0=train(i, r_o,r_a,0);
        
        restore_initialize();
        Q1=train(i, r_o-delta,r_a,1);
        
        restore_initialize();
        Q2=train(i, r_o,r_a+delta,2);
        
        
        printf("i=%d delta=%lf gamma=%lf Q0=%lf Q1=%lf Q2=%lf ro=%lf ra=%lf\n",i,delta, gamma, Q0,Q1,Q2,r_o,r_a);
        r_o=r_o+gamma*(Q0-Q1)/delta;
        r_a=r_a+gamma*(Q2-Q0)/delta;
        
        
        if(i % 200 == 0)
        {
            printf("i=%d r_o=%lf r_a=%lf\n",i,r_o,r_a);
