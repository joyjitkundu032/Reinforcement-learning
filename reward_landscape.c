//Generates the Reward landscape
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include "ran2.c"
...
int main()
{
    ...
    FILE *fpw;
    sprintf(outfile3,"avg_reward_vmax%1.2lft%d.dat",vmax,tstar);
    double delta,gamma,reward;
    double Mx,Mn,Gp;
    Mx=3.0; Mn=1.0; Gp=0.025;
    int dim;
    dim=(Mx-Mn)/Gp;
    ...
    for(i=0;i<Ntrain;i++)
    {
        ...
        printf("i=%d\n",i);
        for(r_o=Mn;r_o<(Mx+eps);r_o=r_o+Gp)
        {
            for(r_a=r_o;r_a<(Mx+eps);r_a=r_a+Gp)
            {
                ...
                //printf("dim=%d col=%d row=%d rw=%lf count=%lf\n",dim,col,row,reward,count[col][row]);
            }
        }
        ...
        for(j=0;j<dim;j++)
        {
            for(k=0;k<dim;k++)
            {
                fprintf(fpw,"%lf %lf %lf\n",j*Gp+Mn,k*Gp+Mn,avg_rw[j][k]/count[j][k]);
                //printf("MIN= %lf MAX=%lf avg=%lf count=%lf\n",MIN[i],MAX[i],avg_rw[j][k],count[j][k]);
            }
            fprintf(fpw,"\n");
        }
        fclose(fpw);
